...
public class BaseTestCase {
    ...
    protected void initSystems() throws Exception {
        StorageSystem storageSystem = (StorageSystem) getNewInstanceOfRemoteSystem(RemoteSystemType.STORAGE, "sftp");
...
//        systemDao.persist(executionSystem);
    ...
    throws Exception
    {
        ...
        if (type == RemoteSystemType.STORAGE) {
            ...
            json = getTestDataObject(STORAGE_SYSTEM_TEMPLATE_DIR + File.separator +
                    filename + ".example.com.json");
        }
        else {
            json = getTestDataObject(EXECUTION_SYSTEM_TEMPLATE_DIR +
     ...
     */
    public JSONObject getTestDataObject(String testDataFilePath) throws JSONException, IOException
    {
        InputStream in = null;
        try
        {
            in = BaseTestCase.class.getClassLoader().getResourceAsStream(testDataFilePath);
            String json = null;
            if (in != null) {
                json = IOUtils.toString(in);
            }
            else {
                throw new FileNotFoundException("No file found for path, " + testDataFilePath + " in classloader.");
            }

            return new JSONObject(json);
        }
        catch (IOException e) {
            log.error("Failed to read test data object for " + testDataFilePath);
            throw e;
        }
        catch (JSONException e) {
            log.error("Failed to parse JSON content from " + testDataFilePath);
            throw e;
        }
        finally {
