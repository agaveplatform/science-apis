...
public class MaverickSSHSubmissionClient implements RemoteSubmissionClient 
{
	...
	private boolean authenticate(int soTimeoutMillis) throws RemoteExecutionException 
	{
		try
		{
            ...
            // which causes some options to be ignored.
            try {
            ...
            try {ssh2 = (Ssh2Client)con.connect(((SshTransport) new SocketWrapper(transport)), username, true);}
                catch (Exception e) {
            ...
            SshTunnel tunnel;
            try {tunnel = ssh2.openForwardingChannel(hostname, port, "127.0.0.1", 22, "127.0.0.1", 22, null, null);}
                catch (Exception e) {
        ...
        catch (Exception e)
        {
            ...
            if (transport != null) try {transport.close();} catch (Exception e1){}
		...
		try 
		{
			...
			if (authenticate(getTimeoutMillis(attemptKey)))
			{
				if (useTunnel()) 
				{
					...
					try {session = forwardedConnection.openSessionChannel();}
					    catch (Exception e) {
					...
					try {session.requestPseudoTerminal("vt100", 80, 24, 0, 0);}
                        catch (Exception e) {
					...
					try {sessionStarted = session.startShell();}
                        catch (Exception e) {
				...
				else 
				{
					...
					try {session = ssh2.openSessionChannel();}
                        catch (Exception e) {
				...
				try {process = shell.executeCommand(command, true, "UTF-8");}
				    catch (Exception e) {
	...
	public void close()
	{
        ...
        try { if (ssh2 != null) ssh2.disconnect(); } 
            catch (Throwable t) {
                String msg = "ssh2 disconnect failure.";
                log.warn(msg, t);
            }
        try { if (forwardedConnection != null) forwardedConnection.disconnect(); } 
            catch (Throwable t) {
