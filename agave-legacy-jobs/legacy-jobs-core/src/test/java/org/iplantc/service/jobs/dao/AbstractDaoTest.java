...
{	
    ...
    throws Exception 
	{
	    try {
            ...
            job.setArchive(true);
            job.setArchiveSystem(privateStorageSystem.getSystemId());
            job.setArchivePath(username + "/archive/test-job-999");
            
            job.setAppId(software.getUniqueName());
            job.setSystem(exeSystem.getSystemId());
            job.setRemoteQueue(queue.getName());
            job.setMaxHours(Job.runTimeToHours(StringUtils.isEmpty(software.getDefaultMaxRunTime()) ? "00:30:00" : software.getDefaultMaxRunTime()));
            job.setMemoryPerNode((float)((software.getDefaultMemoryPerNode() == null) ? 1 : software.getDefaultMemoryPerNode()));
            job.setNodeCount((int) (( software.getDefaultNodes() == null) ? 1 : software.getDefaultNodes()));
            job.setProcessorsPerNode((int)((software.getDefaultProcessorsPerNode() == null) ? 1 : software.getDefaultProcessorsPerNode()));
            ...
            if (JobStatusType.isExecuting(status) || status == JobStatusType.CLEANING_UP) {
                ...
                job.setRemoteSubmitted(created.plusMinutes(minutesAgoJobWasSubmitted).toDate());
                job.setRemoteStarted(created.plusMinutes(minutesAgoJobWasStarted).toDate());
                ...
                if (status == JobStatusType.CLEANING_UP) {
                    ...
                    job.setEnded(created.plusMinutes(minutesAgoJobWasEnded).toDate());
                }
                ...
                job.setRemoteSubmitted(stagingEnded.toDate());
                job.setRemoteStarted(startTime.toDate());
                job.setEnded(endTime.toDate());
