...
public class RemoteUnaryTransferListenerImpl extends RemoteTransferListenerImpl {
    private static final Logger log = LoggerFactory.getLogger(RemoteUnaryTransferListenerImpl.class);

    public RemoteUnaryTransferListenerImpl(TransferTask transferTask, RetryRequestManager retryRequestManager) {
        super(transferTask, retryRequestManager);
    }

    public RemoteUnaryTransferListenerImpl(TransferTask transferTask, Vertx vertx, RetryRequestManager retryRequestManager) {
    ...
    public RemoteTransferListener createChildRemoteTransferListener(String childSourcePath, String childDestPath) throws TransferException {
        ...
        return new RemoteUnaryTransferListenerImpl(childTransferTask, getVertx(), getRetryRequestManager());
    }
    ...
    public void progressed(long bytesSoFar)
    {
        ...
        if (task != null)
        {
            org.agaveplatform.service.transfers.model.TransferTask transferTask = (org.agaveplatform.service.transfers.model.TransferTask)task;
            TransferStatusType status = transferTask.getStatus();

            if (status.equals(READ_STARTED)) {
                task.setStatusString(READ_IN_PROGRESS.name());
            } else if (status.equals(WRITE_STARTED)){
                task.setStatusString(WRITE_IN_PROGRESS.name());
            }
    ...
    public void completed()
    {
        ...
        if (task != null)
        {
            ...
            org.agaveplatform.service.transfers.model.TransferTask transferTask = (org.agaveplatform.service.transfers.model.TransferTask)task;
            TransferStatusType status = transferTask.getStatus();
            if (status.equals(WRITE_IN_PROGRESS)){
                task.setStatusString(WRITE_COMPLETED.name());
            } else if (status.equals(READ_IN_PROGRESS)){
                task.setStatusString(READ_COMPLETED.name());
            }
