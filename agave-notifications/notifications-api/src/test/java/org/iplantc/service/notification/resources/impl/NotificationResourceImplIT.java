...
import org.iplantc.service.notification.ServletJaxRsApplication;
...
public class NotificationResourceImplIT extends AbstractNotificationTest {
	...
	private static final Logger log = Logger.getLogger(NotificationResourceImplIT.class);
...
//				{ mapper.createObjectNode().put("url", TEST_EMAIL).put("associatedUuid",referenceNotification.getUuid()).set("event", mapper.createObjectNode()), "Object for event should throw exception", true },
	...
	public void updateFailedNotification()
	{
		...
		try
		{
			...
			Assert.assertTrue(json.get("policy").get("saveOnFailure").asBoolean(), "Notification policy save_on_failure field should be updated to true in response from service");
		}
		catch (Exception e) {
			Assert.fail("Unexpected exception thrown during notification update", e);
		}
