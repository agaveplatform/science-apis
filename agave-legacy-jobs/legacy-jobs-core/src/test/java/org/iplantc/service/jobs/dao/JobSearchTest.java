...
//        Integer searchValue = Math.round((endTime - job.getStartTime().getTime()) / 1000);
        int searchValue = (int)((new DateTime(job.getEnded()).getMillis() - new DateTime(job.getRemoteStarted()).getMillis())/1000);
...
//	    for (JobStatusType status: JobStatusType.values()) 
    ...
    {   
        ...
        int searchValue = (int)((new DateTime(job.getEnded()).getMillis() - new DateTime(job.getCreated()).getMillis())/1000);
...
//        Assert.assertEquals(savedJob.getCreated(), job.getCreated(), "Job created time changed after saving.");
        ...
        _doFindMatchingTime(job.getUuid(), job.getOwner(), searchTerm, relativeTimePhrase, new DateTime(job.getCreated()).toDate(), shouldMatch);
    }
    ...
    {   
        ...
        Date searchDate = new DateTime(job.getCreated()).toDate();
        if (!StringUtils.isBlank(relativeTimePhrase)) {
        	searchDate = new StringToTime(relativeTimePhrase, new DateTime(job.getCreated()).toDate());
        	
        }
	    
        _doFindExactTime(job.getUuid(), job.getOwner(), searchTerm, searchDate, new DateTime(job.getCreated()).toDate(), shouldMatch);
    }
