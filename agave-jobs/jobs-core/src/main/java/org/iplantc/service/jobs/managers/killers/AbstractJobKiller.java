...
public abstract class AbstractJobKiller implements JobKiller {
	...
	public void attack() throws JobTerminationException, RemoteExecutionException
	{
		...
		try (RemoteSubmissionClient remoteSubmissionClient = getExecutionSystem().getRemoteSubmissionClient(getJob().getInternalUsername())) {
	 ...
	 * @throws JobMacroResolutionException when the startup script cannot be resolved. This is usually due to the
	...
	/**
	 * Filters the scheduler specific job kill command replacing the {@code ${AGAVE_JOB_LOCALJOB_ID}} macro with the
	 * recorded {@link Job#getLocalJobId()} for the job under question.
	 *
	 * @return the filtered job kill command for the scheduler associated with the job.
	 */
	public String getCommand() {
		return StringUtils.replace(
				getExecutionSystem().getScheduler().getBatchKillCommand(),
