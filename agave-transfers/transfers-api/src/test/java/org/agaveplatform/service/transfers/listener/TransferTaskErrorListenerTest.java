...
//		}).when(tt.getRootTaskId());
		...
		txfrErrorListener.processError(body, resp -> ctx.verify(() -> {
			...
			verify(txfrErrorListener)._doPublishNatsJSEvent(eq("TRANSFER_RETRY"), eq(TRANSFER_RETRY), eq(tt.toJson().put("status", ERROR.name())) );
			ctx.completeNow();
		}));
	}
...
//		AsyncResult<JsonObject> expectedUpdateStatusHandler = getMockAsyncResult(expectedUdpatedJsonObject);
		...
		txfrErrorListener.processError(body, resp -> ctx.verify(() -> {
			...
			verify(txfrErrorListener, never())._doPublishNatsJSEvent(any(), any(), any());
			ctx.completeNow();
		}));
	}
