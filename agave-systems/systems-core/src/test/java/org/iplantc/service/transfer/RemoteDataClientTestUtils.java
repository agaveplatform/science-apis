...
public abstract class RemoteDataClientTestUtils extends BaseTransferTestCase
{
    ...
    protected Object[][] mkdirProvider()
    {
        return new Object[][] {
            ...
            { UUID.randomUUID() + "/" + UUID.randomUUID(), false, true, "mkdir when parent does not exist should throw exception." },
        };
    }
    ...
    protected Object[][] mkdirsProvider()
    {
        return new Object[][] {
            ...
            { UUID.randomUUID() + "/" + UUID.randomUUID(), false, "mkdirs when parent does not exist should not throw an exception." },
        };
    }
    ...
    protected Object[][] doesExistProvider()
    {
        return new Object[][] {
            ...
            { UUID.randomUUID() + "/" + UUID.randomUUID(), false, "Missing directory should not return true from doesExist." },
        };
    }
...
//                    "Failed to upload test file for put merge test.");
    ...
    protected void _putFileFailsToMissingDestinationPath()
    {
        try
        {
            getClient().put(LOCAL_BINARY_FILE, UUID.randomUUID() + "/" + MISSING_DIRECTORY);
    ...
    protected void _putFolderFailsToMissingDestinationPath()
    {
        try
        {
            getClient().put(LOCAL_DIR, UUID.randomUUID() + "/" + MISSING_DIRECTORY);
    ...
    protected void _putFailsForMissingLocalPath()
    {
        ...
        try
        {
            ...
            getClient().put(UUID.randomUUID() + "/" + MISSING_DIRECTORY, remoteBaseDir);
    ...
    protected void _deleteFailsOnMissingDirectory()
    {
        try
        {
            getClient().delete(UUID.randomUUID() + "/" + MISSING_DIRECTORY);
    ...
    protected void _isDirectoryTrueForDirectory()
    {
        try
        {
            ...
            try {
                getClient().isDirectory(UUID.randomUUID() + "/" + MISSING_DIRECTORY);
    ...
    protected void _isFileFalseForDirectory()
    {
        try
        {
            ...
            try {
                getClient().isFile(UUID.randomUUID() + "/" + MISSING_FILE);
                Assert.fail("Non-existent folder should throw exception");
            } catch (Exception e) {
                Assert.assertTrue(true);
            }
    ...
    protected void _lsFailsOnMissingDirectory()
    {
        try
        {
            getClient().ls(UUID.randomUUID() + "/" + MISSING_DIRECTORY);
    ...
    protected void _getThrowsExceptionOnMissingRemotePath()
    {
        ...
        try
        {
            ...
            getClient().get(UUID.randomUUID() + "/" + MISSING_DIRECTORY,
                    localDir.toString());
                ...
                if (expectedDownloadPath != null) {
                    if (Files.exists(expectedDownloadPath) && expectedDownloadPath.toFile().isFile()) {
                        ...
                        Assert.assertTrue(FileUtils.deleteQuietly(expectedDownloadPath.toFile()),
                                "Failed to cleanup expected download path " + expectedDownloadPath);
                    }
                }
                ...
                if (localPath != null) {
                    if (Files.exists(localPath) && localPath.toFile().isFile()) {
                        ...
                        Assert.assertTrue(FileUtils.deleteQuietly(localPath.toFile()),
                                "Failed to cleanup local test dir " + localPath);
                    }
                }
            } catch(Throwable ignore) { }
...
//            getClient().syncToRemote(tempLocalDir, "", new RemoteTransferListener(task));
    ...
    protected void _getFileInfoReturnsErrorOnMissingPath() throws FileNotFoundException {
        ...
        {
            remoteBaseDir = createRemoteTestDir();
            String remotePath = String.format("%s/%s/%s", remoteBaseDir, UUID.randomUUID(), MISSING_DIRECTORY);
