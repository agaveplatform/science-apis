...
public class CallbackMacroResolver {
	...
	public static String resolve(Job job, String value)
	{
		...
		value = value.replaceAll("(?i)\\$\\{JOB_START_TIME\\}", job.getRemoteStarted().toString());
		
		value = value.replaceAll("(?i)\\$\\{JOB_END_TIME\\}", job.getEnded().toString());
		
		value = value.replaceAll("(?i)\\$\\{JOB_SUBMIT_TIME\\}", job.getRemoteSubmitted().toString());
		
		value = value.replaceAll("(?i)\\$\\{JOB_ARCHIVE_SYSTEM\\}", 
				(job.getArchiveSystem() == null ? "null" : job.getArchiveSystem()));
		...
		value = value.replaceAll("(?i)\\$\\{JOB_ERROR\\}", 
				(ServiceUtils.isValid(job.getLastStatusMessage()) ? job.getLastStatusMessage() : ""));
