...
//
//		try {
//			agaveClient.listen(queue, topic, listener);
//
...
//	}
	...
	public void pop(String message, boolean shouldThrowException, String errorMessage)
	{
		...
		try
		{
			...
			Assert.assertEquals(jobid, poppedMessage.getId(), "Retrieved wrong message from queue");
			Assert.assertEquals(poppedMessage.getMessage(), message);

			agaveClient.delete(TEST_EXCHANGE_TOPIC, TEST_EXCHANGE_TOPIC_QUEUE, poppedMessage.getId());
		}
		catch (Exception e)
		{
			if (!shouldThrowException) {
	...
	public void popMultiple()
	{
		...
		try
		{
			...
			for(int i=0;i<5;i++)
			{
				...
				Assert.assertNotNull(poppedMessage, "No message popped from the queue.");
				Assert.assertNotNull(poppedMessage.getId(), "No message id returned.");
				Assert.assertEquals(notifs.get(i), poppedMessage.getId(), "Retrieved wrong message from queue");
	...
	public void reject()
	{
		...
		try
		{
			...
			agaveClient.reject(TEST_EXCHANGE_TOPIC, TEST_EXCHANGE_TOPIC_QUEUE, msg.getId(), message);
