...
//		NatsJetstreamMessageClient nats = getMockNats();
    ...
    @Test
    @DisplayName("TransferTaskAssignedListener - promise should fail if exception is thrown during start")
    public void testFailIfExceptionThrownDuringStart(Vertx vertx, VertxTestContext ctx) throws Exception {
        TransferTaskAssignedListener listener = getMockTransferAssignedListenerInstance(vertx);
        ...
        try {
            doThrow(new InterruptedException("Promise should fail when exception is thrown during start")).when(listener).subscribeToSubjectGroup(eq(TRANSFERTASK_ASSIGNED), any(Handler.class));
        } catch (Exception e) {
            try {
