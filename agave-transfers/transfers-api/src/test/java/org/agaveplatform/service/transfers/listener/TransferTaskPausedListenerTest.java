...
//		});
	...
	@DisplayName("A test")
	@Disabled
	public void testTransferAssigned(Vertx vertx, VertxTestContext ctx){
		// mock out the test class
		TransferTaskAssignedListener ta = getMockTransferAssignedListenerInstance(vertx);
...
//			handler.handle(updatedTransferTaskAsyncResult);
		...
		doCallRealMethod().when(ta).addPausedTask( any()); // eq(rootTransferTask.getUuid()) ) ;
		//doCallRealMethod().when(ta).pausedTasks.contains(eq(rootTransferTask.getUuid()));

		//ta.start();

		vertx.eventBus().send(TRANSFERTASK_PAUSED_SYNC, rootTransferTaskJson);

		doCallRealMethod().when(ta).addPausedTask(rootTransferTask.getUuid() );

		//ta.addPausedTask(rootTransferTask.getUuid());


		DeploymentOptions options = new DeploymentOptions().setConfig(config);
		vertx.deployVerticle(TransferTaskAssignedListener.class.getName(), options, assignedId -> {
			if (assignedId.succeeded()) {
				ctx.verify(() -> {
					//			doCallRealMethod().when(ta).addPausedTask(anyString());
					//assertEquals(true, TransferTaskAssignedListener.checkPausedTask(rootTransferTask.getUuid()));
					assertTrue(ta.pausedTasks.contains(rootTransferTask.getUuid()));
					ctx.completeNow();
				});
			}
		});
