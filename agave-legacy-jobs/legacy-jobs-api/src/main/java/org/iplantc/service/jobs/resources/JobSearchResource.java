...
public class JobSearchResource extends SearchableAgaveResource<JobSearchFilter> {
	...
	//				Job job = jobs.get(i);
					writer.object()
						...
						.key("startTime").value(job.getRemote_started() == null ? null : new DateTime(job.getRemote_started()).toString())
						.key("endTime").value(job.getEnded() == null ? null : new DateTime(job.getEnded()).toString())
						.key("_links").object()
				        	.key("self").object()
