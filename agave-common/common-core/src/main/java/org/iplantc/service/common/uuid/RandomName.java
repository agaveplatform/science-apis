...
public class RandomName {
     ...
     * formatted as "adjective_surname". For example 'focused_turing'. If retry is true, a random
     * integer between 0 and 10 will be added to the end of the name, e.g `focused_turing3`
     * @param retry true if a random natural number less than 10 should be appended to the name
     */
    public String next(boolean retry) {
        String name = "";
        Random rand = new Random();
        do {
            name = String.format("%s_%s", left[rand.nextInt(left.length)], right[rand.nextInt(right.length)]);
        } while(name.equals("boring_wozniak"));/* Steve Wozniak is not boring */

        if (retry) {
            name = String.format("%s%d", name, rand.nextInt(10));
        }
        return name;
    }
}
