...
public enum WrapperTemplateAttributeVariableType implements WrapperTemplateVariableType
{
	...
	public String resolveForJob(Job job)
	{
		...
		{
			String queueName = job.getRemoteQueue();
			try {
				ExecutionSystem system = (ExecutionSystem)new SystemDao().findBySystemId(job.getSystem());
				if (system != null) {
					BatchQueue queue = system.getQueue(job.getRemoteQueue());
					if (queue != null) {
		...
		{
			return job.getRemoteSubmitted().toString() ;
		}
		else if (this == AGAVE_JOB_ARCHIVE_SYSTEM)
		{
			return (job.getArchiveSystem() == null ? "" : job.getArchiveSystem());
		}
		...
		{
			return String.valueOf(job.getMaxHours());
		}
		else if (this == AGAVE_JOB_MAX_RUNTIME_MILLISECONDS)
		{
			try {
				return String.valueOf(TimeUtils.getMillisecondsForMaxTimeValue(job.hoursToRunTime(job.getMaxHours())));
			}
			catch (Exception e) {
				return "0";
			}
		...
		{
			try {
				return String.valueOf(Math.floor(TimeUtils.getMillisecondsForMaxTimeValue(job.hoursToRunTime(job.getMaxHours()))/1000));
			}
			catch (Exception e) {
				return "0";
			}
