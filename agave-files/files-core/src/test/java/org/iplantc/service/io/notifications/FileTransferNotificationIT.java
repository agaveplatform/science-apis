...
public class FileTransferNotificationIT extends BaseTestCase {
    ...
    @BeforeAll
    protected void beforeClass(Vertx vertx, VertxTestContext ctx) throws Exception {
        super.beforeClass();
    ...
    @AfterAll
    protected void afterClass(Vertx vertx, VertxTestContext ctx) throws Exception {
        clearSystems();
        clearLogicalFiles();
    }

    @BeforeEach
    protected void setUp(Vertx vertx, VertxTestContext ctx) throws Exception {
        clearLogicalFiles();
    ...
    public void fileTransferNotificationTest(Vertx vertx, VertxTestContext ctx) throws SchedulerException {
        ...
        try {
            ...
            vertx.deployVerticle(FilesTransferListener.class.getName(), new DeploymentOptions(), ctx.succeeding(filesListenerId -> {
                ...
                try {
                    vertx.eventBus().request("TRANSFER_COMPLETED", new JsonObject(file.toJSON()), response -> {
                    ...
                    Assertions.fail("Unable to get JsonObject of logical file: " + e.getMessage());
                }
            }));

            //Verify that the notification sent onto the queue is correct
            List<Notification> notifications = notificationDao.getActiveForAssociatedUuidAndEvent(file.getUuid(), FileEventType.STAGING_COMPLETED.name());
            ...
            Assert.assertTrue(n.getUuid() == file.getUuid(), "Notification for transfer " + file.getTransferUuid() + " was successfully created");
