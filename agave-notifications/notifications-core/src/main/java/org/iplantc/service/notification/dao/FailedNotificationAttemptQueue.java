...
//                Settings.METADATA_DB_USER, "api", Settings.METADATA_DB_PWD.toCharArray());
	...
	throws NotificationException
	{
		try {
            ...
            //cannot directly remove item from capped collection
            //collection must be dropped then re-added
            DBCursor cursor = cappedCollection.find(query).limit(1);
            if (!cursor.hasNext())
                throw new MongoException("Notification Attempt does not exist.");

            DBObject toRemove = cursor.next();
            List<DBObject> currentCollection = cappedCollection.find().toArray();
            currentCollection.remove(toRemove);
            cappedCollection.drop();
            cappedCollection.insert(currentCollection);
		}
		catch (MongoException e) {
	...
	throws NotificationException
	{
		try {
			...
			if (cursor.hasNext()) {
				...
				return mapper.readValue(strResult, NotificationAttempt.class);
			}
		}
		catch (JsonProcessingException e) {
