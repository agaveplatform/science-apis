...
public class DataLocatorTest
{
	...
	throws Exception
	{
		...
		finally
		{
			...
			if (job.isArchive())
			{
				deleteData(systemDao.findBySystemId(job.getArchiveSystem()), job.getArchivePath() + "/" + "data");
			}
	...
	throws Exception
	{
		...
		job.setArchive(shouldArchive);
		job.setArchivePath(archiveSystem.getStorageConfig().getHomeDir());
		job.setArchiveSystem(archiveSystemId);
		job.setCreated(new Date());
		job.setMemoryPerNode((float)512);
		job.setOwner("dooley");
		job.setProcessorsPerNode(1);
		job.setMaxHours(Job.runTimeToHours("1:00:00"));
