...
@Test(groups={"unit"})
public class TorqueXmlJobStatusResponseParserTest extends AbstractJobStatusResponseParserTest {

	protected final static String TEST_SCHEDULER_JOB_ID = "1125";
	protected final static String TEST_USER = "testuser";
	protected final static String TEST_STATUS_QUERY_RESPONSE_TEMPLATE_PATH = "schedulers/torque/qstat.xml";
	 ...
	 * @return a formatted job status response with the given values injected
	 */
	private String getSchedulerResponseString(String testSchedulerJobId, TorqueJobStatus testSchedulerJobStatus, String testSchedulerJobExitCode) {
	 ...
	 * @param testSchedulerJobStatus the status to set in the returned job status response
	 * @return a formatted job status response with the given values injected
	 * @see #getSchedulerResponseString(String, TorqueJobStatus, String)
	 */
	private String getSchedulerResponseString(String testSchedulerJobId, TorqueJobStatus testSchedulerJobStatus) {
	...
	{
		return new TorqueXmlJobStatusResponseParser();
	}
	...
	{
		return new Object[][] {
				{ getSchedulerResponseString(getTestSchedulerJobId(), TorqueJobStatus.WAITING).replaceAll("\\s+", "|"), "Pipe delimited server response should throw RemoteJobMonitorResponseParsingException" },
				{ getSchedulerResponseString(getTestSchedulerJobId(), TorqueJobStatus.WAITING).replaceAll("\\s+", ","), "Comma delimited server response should throw RemoteJobMonitorResponseParsingException" },
				{ getSchedulerResponseString(getTestSchedulerJobId(), TorqueJobStatus.WAITING).replaceAll("\\s+", "|"), "Pipe delimited server response should throw RemoteJobMonitorResponseParsingException" },
				{ getSchedulerResponseString(getTestSchedulerJobId(), TorqueJobStatus.WAITING).replaceAll("\\s+", ","), "Comma delimited server response should throw RemoteJobMonitorResponseParsingException" },

				{ TorqueJobStatus.WAITING.getCode(), "Status only response should throw RemoteJobMonitorResponseParsingException" },
		};
	}
	...
	protected Object[][] parseSchedulerResponseProvider()
	{
		...
		testCases.add(new Object[]{ getSchedulerResponseString(getTestSchedulerJobId(), "UNKNOWN"), TorqueJobStatus.UNKNOWN });
		testCases.add(new Object[]{ getSchedulerResponseString(getTestSchedulerJobId(), "asdfasdfasdfadfa"), TorqueJobStatus.UNKNOWN });
...
//		_parseCaseInsensitiveSchedulerResponse(rawServerResponse.toLowerCase(), expectedStatus);
	...
	{
		return new Object[][] {
				{getSchedulerResponseString("", TorqueJobStatus.SUSPENDED)},
				{getSchedulerResponseString("foo-" + getTestSchedulerJobId() , TorqueJobStatus.RUNNING)},
				{getSchedulerResponseString(getTestSchedulerJobId() + "-foo" , TorqueJobStatus.WAITING)},
				{getSchedulerResponseString("foo-" + getTestSchedulerJobId() + "-foo" , TorqueJobStatus.WAITING)},
