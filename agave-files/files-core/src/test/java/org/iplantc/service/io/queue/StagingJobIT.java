 ...
 * //TODO: we can recycle some of the permission tests for the transfers api to
 */
@Deprecated
@Test(singleThreaded = true, groups = {"integration","broken"}, enabled = false)
...
//	throws Exception
//	{
//        String systemId = UUID.randomUUID().toString();//String.format("%s%s%s", protocol, setPublic ? "-public" : "", setDefault ? "-default" : "");
...
//	{
//        return new Object[][]{
////                {gridftpSystem, "/root", "", "Staging should fail when user does not have permission on source path"},
//                {"irods", "/testotheruser", "", "Staging should fail when user does not have permission on source path"},
//                {"sftp", "/root", "", "Staging should fail when user does not have permission on source path"},
////                {ftpSystem, "/root", "", "Staging should fail when user does not have permission on source path"},
////                {s3System, "/", "", "Staging should fail when user does not have permission on source path"},
//        };
//    }
//
...
//	public void testStageNextAgaveFileSourceNoPermission(String sourceSystemProtocol, String srcPath, String destPath, String message)
//	{
//        RemoteDataClient destClient = null;
//		try
//		{
//            // load the db with dummy records to stage a http-accessible file
//            RemoteSystem destSystem = initSystem("sftp", true, true);
//            RemoteSystem sourceSystem = initSystem(sourceSystemProtocol, false, false);
//
//            destClient = destSystem.getRemoteDataClient();
//            destClient.authenticate();
//
...
//		}
//		finally {
//			try { if (destClient != null) destClient.delete(""); } catch (Exception ignored) {}
//        }
//    }
//
//    @DataProvider(parallel = false)
