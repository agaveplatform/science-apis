	...
	throws Exception
	{
		try {
			...
			Mockito.doReturn(Boolean.TRUE)
					.when(manager)
					.validateSoftwareDependencies(Mockito.any(String.class), Mockito.any(Software.class));
			
			Software newSoftware = manager.processSoftware(oldSoftware, json, oldSoftware.getOwner());
			
			Assert.assertEquals(newSoftware.getUuid(), oldSoftware.getUuid(),
					"UUID should be the same before and after processing");
			
			Assert.assertEquals(newSoftware.getCreated().toString(), oldSoftware.getCreated().toString(),
					"created timestamp should be the same before and after processing");
			...
			Assert.assertNull(oldSoftwareResult,
					"Search for old software by id should return null.");
			
			Assert.assertNotNull(newSoftwareResult,
					"Search for new software by id should return new software.");
			...
			for (SoftwarePermission oldPem : oldSoftware.getPermissions()) {
				Assert.assertTrue(newSoftwareResult.getPermissions().contains(oldPem),
						"All permissions should be carried over after processing.");
			}
		}
