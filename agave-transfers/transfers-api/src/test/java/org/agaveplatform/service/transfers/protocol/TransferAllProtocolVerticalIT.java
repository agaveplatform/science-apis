...
class TransferAllProtocolVerticalIT extends BaseTestCase {
	...
	public void testProcessCopyRequestInterruptsURLCopy (Vertx vertx, VertxTestContext ctx) throws SystemUnknownException, AgaveNamespaceException, RemoteCredentialException,
		...
		txfrAllVert.processCopyRequest(srcRemoteDataClient, destRemoteDataClient, tt, ctx.failing(interruptException -> ctx.verify(() -> {
			...
			verify(txfrAllVert, atLeastOnce())._doPublishEvent(eq(MessageType.TRANSFERTASK_CANCELED_ACK), anyObject());
    ...
    public void testProcessCancelSync(Vertx vertx, VertxTestContext ctx) throws RemoteDataException, RemoteCredentialException, IOException, SystemUnknownException, AgaveNamespaceException, PermissionException, TransferException, RemoteDataSyntaxException {
        ...
        txfrAllVert.processEvent(tt.toJson(), processEventResp -> ctx.verify(() -> {
				...
				verify(txfrAllVert)._doPublishEvent(eq(TRANSFERTASK_CANCELED_ACK), any());
				ctx.completeNow();
			}));
    }
